...
export default function ProfilesPage() {
  ...
  const fetchProfiles = async () => {
    ...
    setLoading(true);
    try {
      const data = await getProfiles(user.id, page, perPage);
    ...
    <div>
      <div style={{ marginBottom: "40px" }}>
        ...
        <p style={{ color: "var(--text-secondary)", fontSize: "0.875rem", marginTop: "8px" }}>
          {total} profile{total !== 1 ? "s" : ""} in your vault
        </p>
      </div>
      ...
      {loading ? (
        ...
        >
          {Array.from({ length: 8 }).map((_, i) => (
            <div
              key={i}
              style={{ background: "var(--surface)", height: "220px", padding: "20px" }}
            >
              <div
                style={{
                  width: "100%",
              ...
              />
              <div style={{ height: "12px", background: "var(--surface-raised)", width: "60%" }} />
            </div>
          ))}
        </div>
      ) : profiles.length === 0 ? (
        ...
        <>
          ...
          >
            {profiles.map((profile) => (
              <div
                ...
                style={{ background: "var(--surface)", padding: "16px", gap: "12px" }}
              >
                <div
                  style={{
                    width: "100%",
          ...
          {totalPages > 1 && (
            <div
              ...
              style={{ borderColor: "var(--gold-border)", marginTop: "32px", paddingTop: "24px" }}
            >
              <button
                ...
                style={{
                  color: page === 1 ? "var(--text-muted)" : "var(--gold)",
                  background: "transparent",
                  border: "none",
              ...
              <button
                ...
                className="text-xs tracking-widest uppercase"
                style={{
                  color: page === totalPages ? "var(--text-muted)" : "var(--gold)",
                  background: "transparent",
                  border: "none",
